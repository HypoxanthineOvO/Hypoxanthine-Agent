...
metadata:
  ...
  hypo.backend: "notion"
  hypo.exec_profile:
  hypo.triggers: "notion,笔记,页面,数据库,记录,写入,创建页面,查询,工作区,写到notion,读取页面,计划通,待办,事项,任务"
  hypo.risk: "medium"
...
3. 改正文走 `notion_write_page`，改属性走 `notion_update_page`。
4. 处理 database 行时，用 `notion_query_db` 或 `notion_create_entry`。
5. 如果用户没有精确 ID，从 `notion_search` 开始定位对象。
