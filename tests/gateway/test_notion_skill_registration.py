...
def test_build_default_deps_skips_notion_skill_when_service_config_missing(
    tmp_path: Path,
    monkeypatch,
) -> None:
...
""".strip(),
    ...
    assert "notion" not in deps.skill_manager._skills
    assert logger.warning_calls == [
        (
            "notion_skill.disabled",
            {
                "reason": (
                    "Missing Notion config: config/secrets.yaml -> "
                    "services.notion.integration_secret"
                )
            },
        )
    ]
